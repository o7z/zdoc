...
每个想出现在侧边栏的目录都要放一个 `_meta.yaml`。它声明这个目录自己的标题和排序，以及目录下要被收录到侧边栏的文档列表。
...
只有列在 `pages` 里的 `.md` 文件才能被路由访问。**没列出来 = 404 + 不出现在侧边栏。**
...
- 目录节点本身**不可点击**，只能折叠/展开。如果想给某个分组弄一个入口页，请在 `pages` 里加一条目即可。
