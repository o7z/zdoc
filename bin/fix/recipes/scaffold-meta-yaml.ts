...
import type { Recipe, Finding, DocsScan, DirMeta } from '../types.js';
...
export interface ScaffoldMetaYamlPayload {
	/** The directory that is missing its _meta.yaml */
	dirPath: string;
	/**
	 * Signals to the engine that finding.file does not yet exist on disk.
	 * The engine must bypass the SHA-existence check and pass '' as before.
	 */
	isNewFile: true;
}
...
const recipe: Recipe<ScaffoldMetaYamlPayload> = {
	id: 'scaffold-meta-yaml',
	autoFix: true,
	description: '为只有 .md 但缺 _meta.yaml 的目录生成 _meta.yaml',

	detect(scan: DocsScan, _sources: Map<string, string>): Finding<ScaffoldMetaYamlPayload>[] {
		const findings: Finding<ScaffoldMetaYamlPayload>[] = [];
	...
	apply(finding: Finding<ScaffoldMetaYamlPayload>, _before: string): string {
		...
		for (const fileName of entries) {
			if (fileName === 'index.md') continue; // whitelisted, skip
