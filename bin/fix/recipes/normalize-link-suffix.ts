...
// for manual review.
const KNOWN_EXTS = ['.md', '.pdf'] as const;

interface Payload {
	mdFile: string;       // absolute path of the .md being fixed (== finding.file)
	rawLink: string;      // the raw `[text](path)` form (the `(path)` part)
	suggested: string;    // the path with the chosen extension added
}
...
function hasKnownExt(pathOnly: string): boolean {
...
function findCandidates(absBaseNoExt: string): Candidate[] {
	const out: Candidate[] = [];
...
const recipe: Recipe<Payload> = {
		...
		for (const mdFile of scan.mdFiles) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				while ((m = LINK_RE.exec(stripped)) !== null) {
					...
					if (!pathOnly) continue; // pure anchor
					if (hasKnownExt(pathOnly)) continue; // already explicit
					...
					if (cands.length === 0) continue; // broken — internal-link lint handles it
					if (cands.length === 1) {
	...
	apply(finding: Finding<Payload>, before: string): string {
		...
		if (!suggested) return before; // manualReview — should never reach here
		...
		if (idx === -1) return before; // link no longer present (already applied?)
