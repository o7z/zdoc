// Minimal _meta.yaml parser for bin/ tools (lint, mcp).
//
// Mirrors src/lib/meta.ts deliberately: bin/'s tsconfig restricts
// rootDir to bin/, so the SvelteKit-side parser cannot be imported.
// Keep this file in sync with src/lib/meta.ts — primarily the YAML
// rule that '#' is a comment only when at line start or preceded by
// whitespace (so values like /docs/foo.md#section parse correctly).
