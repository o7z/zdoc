...
//
// Self-contained: ships its own minimal _meta.yaml parser to avoid cross-dir
// imports (bin/ has rootDir: "bin" in tsconfig.bin.json). The parser logic
// mirrors src/lib/meta.ts and must be kept in sync — primarily the YAML rule
// that '#' is a comment only when at line start or preceded by whitespace.

import { readFileSync, readdirSync, existsSync, statSync } from 'node:fs';
import { join, resolve, dirname, relative, sep } from 'node:path';

// -----------------------------------------------------------------------------
// Mini YAML parser (mirrors src/lib/meta.ts; keep in sync)
// -----------------------------------------------------------------------------
...
function parseYaml(input: string): Record<string, unknown> {
	...
	if (lines[0].indent !== 0) throw new Error(`Root line must start at column 0`);
...
function readDirMeta(metaPath: string): DirMeta | null {
	if (!existsSync(metaPath)) return null;
	try {
		const raw = parseYaml(readFileSync(metaPath, 'utf-8'));
		const out: DirMeta = {
		...
		if (pagesRaw && typeof pagesRaw === 'object') {
			...
			for (const [k, v] of Object.entries(pagesRaw as Record<string, unknown>)) {
				...
				pages[k] = {
					...
					env: typeof p.env === 'string' ? p.env : undefined,
					lifecycle: typeof p.lifecycle === 'string' ? p.lifecycle : undefined,
					superseded_by: typeof p.superseded_by === 'string' ? p.superseded_by : undefined,
					folded_to: typeof p.folded_to === 'string' ? p.folded_to : undefined,
				};
			}
			out.pages = pages;
		}
