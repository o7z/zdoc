...
子目录里的 `index.md` 在 lint 中有**孤儿豁免** —— 即使没登记到任何 `_meta.yaml.pages` 里，也不会被报为孤儿，可以直接作为目录着陆页存在，从外部（首页 hero、兄弟文档链接）跳进来。

但要注意：**父目录的 `_meta.yaml.pages` 里的 key 永远解析为同级 `<key>.md`，不会回退到 `<key>/index.md`**。如果你在根 `_meta.yaml` 里写 `overview:` 期望它命中 `overview/index.md`，lint 会报"`overview.md` 不存在"。让 `overview/` 子目录有自己的 `_meta.yaml`（带 `title:`）即可让它出现在侧边栏；详见 [`_meta.yaml` 的"注意"段](/guide/authoring/meta-yaml.md#注意)。
