...
| **MINOR** | 新功能：新增 CLI 选项/命令；新增文档渲染能力；新增独立 UI 功能；向后兼容的架构改进（如文档结构重组、更新机制重写）；向后兼容的性能优化 |
| **PATCH** | 修复：Bug 修复；样式/排版微调；文档内容修正；依赖安全更新；不改变用户可见行为的代码改进；为修复 Bug 而新增的辅助 UI（如 loading 态为空状态提示） |
